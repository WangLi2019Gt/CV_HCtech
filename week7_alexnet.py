...
def get_flops(layer,fea):
    ...
    #pdb.set_trace()
    # 这里填入flops计算方法：
    flops =layer.in_channels*layer.out_channels*layer.kernel_size[0]*layer.kernel_size[1]*fea.shape[2]*fea.shape[3]
    flops = float(flops)
    print_f(layer)
...
def print_params_num(layers):
    ...
    for i in [0,2,4,5,6,8,9,10]: 
        ...
        if i ==8 or i==9 or i==10:
            layer_num = layers[i].in_features*layers[i].out_features+layers[i].out_features
        else:
            layer_num = layers[i].in_channels*layers[i].out_channels*layers[i].kernel_size[0]*layers[i].kernel_size[1]+layers[i].out_channels
        # 换算为k
...
if __name__=="__main__":
    ...
    layers.append(conv6)
    # 填写输入，输出通道数
    conv7=nn.Conv2d(384 ,384 ,kernel_size = 3,stride=1,padding=1)
    layers.append(conv7)
    # 填写kernel_size 和stride
    pool8=nn.MaxPool2d(kernel_size=4 , stride=2 , padding=0)
    layers.append(pool8)
    # 填写输入，输出神经元数
    fc9 = nn.Linear(9600 ,4096 )
    layers.append(fc9)
